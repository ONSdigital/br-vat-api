 ...
 */
class VatEditModule extends AbstractModule {
  ...
  @Provides
  def providesPatchService(@Inject() commandRepository: CommandRepository[VatRef, Ubrn],
                           unitRegistryService: UnitRegistryService[Ubrn])
                          (implicit ec: ExecutionContext): PatchService[VatRef] =
    new AdminDataPatchService[VatRef](commandRepository, unitRegistryService)
